...
fun RegisterScreen(userViewModel: UserViewModel, onSignUpSuccess: () -> Unit) {
    ...
    Column(
        ...
        Button(
            ...
            onClick = {
                // Validate user input checking for empty values
                if (fName.isNotBlank() && lName.isNotBlank()) {
                    disNameError = false
                } else if (fName.isBlank() && lName.isBlank()) {
                    disNameError = true
                }
                // Ensure email fits pattern
                if (android.util.Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
                    disEmailError = false
                } else if (!android.util.Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
                    disEmailError = true
                }
                // Use regex to validate password length and characters
                ...
                if (fName.isNotBlank() && lName.isNotBlank() && android.util.Patterns.EMAIL_ADDRESS.matcher(
                        email
                    ).matches()
                    && Regex("^[a-zA-Z0-9@_]+$").matches(password) && password.length >= 8
                ) {
                    // Register user using method defined in user class
                    val user =
        ...
        if (disNameError) Text(
            text = "Please enter your first and last name.",
            color = MaterialTheme.colorScheme.error
        )
        if (disEmailError) Text(
            text = "Please enter a valid email address.",
            color = MaterialTheme.colorScheme.error
        )
        if (disPasswordError) Text(
            text = "Password must be at least 8 characters and can only contain letters, numbers, @, and _",
            textAlign = TextAlign.Center,
            color = MaterialTheme.colorScheme.error
        )
